...
contract Dreamland is JuiceboxProject, ERC721Enumerable {
    ...
    // Mint Dream for `_tileAddress` to `msg.sender`
    function mint(address _tileAddress) external payable returns (uint256) {
        require(saleIsActive, "Sale has not started");
        require(totalSupply() < MAX_SUPPLY, "All dreams have been minted.");
        require(idOfAddress[_tileAddress] == 0, "Tile has already been dreamt");
        require(
            tiles.ownerOf(tiles.idOfAddress(_tileAddress)) == msg.sender,
        ...
        _takeFee(
            ...
            msg.sender,
            string(
                abi.encodePacked("Dreamt of Tile ", toAsciiString(_tileAddress))
            ),
            false
        );

        return _mint(msg.sender, _tileAddress);
    }

    function _mint(address to, address _tileAddress) private returns (uint256) {
        // Start IDs at 1
        ...
        emit Mint(to, tokenId, _tileAddress);
